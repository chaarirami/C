 ...
 * \version 4.0
 * \date    2018-05-04
 * changes: Macros for equal work with Virtual-C
  */
...
if(fabs((e)-(f))<EXPECT_EPSILON){0;}else{printf("expectation %s (%f) approximately equals %s fails, in %s, line %i\n", #e, e, #f,__FILE__, __LINE__);} \
} while(0)

#define EXPECT_APPROX_EQUAL_MESSAGE(m,e,f) do{ \
if(fabs((e)-(f))<EXPECT_EPSILON){0;}else{printf(m ", expectation %s (%f) approximately equals %s fails, in %s, line %i\n", #e, e, #f, __FILE__, __LINE__);} \
} while(0)
